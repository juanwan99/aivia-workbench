...
1. 新开对话，**先读本文全文** + `docs/PROJECT-PLAN.md`  
...
已拍板: OpenWork+DeepSeek；不改 Agent 核；空成功禁止
阻塞: <无 / 列出>
不做什么: Pico 大包、Dify 改主线、拆 WB
请业主: <仅当需要授权时>
```
...
| 平台能力可迁教育 | 假「教育专用仓」滤掉强平台（已选 OpenWork 则锁主线） |
...
```text
1) 读 docs/HANDOFF.md + PROJECT-PLAN.md + CHANGE-POLICY.md
2) 按 §5 打状态报告
3) 派或自执行：安装 OpenWork + DeepSeek + G1
4) 结果写入 aivia-workbench#1 与 upstream/PIN.md
5) G1 绿后才开阶段 B（交付规则 + skills）
```
...
**交接完成条件：** 新窗已用 §5 打出首报，且承认下一刀 = OpenWork 安装与 G1。
