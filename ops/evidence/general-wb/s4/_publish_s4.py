#!/usr/bin/env python3
"""Publish S4: multi-deliverable PackDownload + system prompt. No secrets printed."""
...
PACK_SRC = Path("/tmp/packdownload_s4.py")
SYS_SRC = Path("/tmp/general-wb-s4-system.txt")
...
def main() -> None:
    ...
    if "_pack_multi" not in pack_code:
        raise SystemExit("s4 multi missing in pack")
    ...
    sql = (
        ...
        "  created_by, now(), updated_by, now(), 'general-wb-s4', 'S4-CAP multi pack+prompt',\n"
        "  environment_variables, conversation_variables\n"
    ...
    Path("/tmp/publish_s4.sql").write_text(sql, encoding="utf-8")
    with open("/tmp/publish_s4.sql", "rb") as f:
    ...
    Path("/tmp/general-wb-s4-wf-id.txt").write_text(new_id, encoding="utf-8")
    print("NEW_WF", new_id)
    print(
        "APP",
    ...
    for n in g2["nodes"]:
        ...
        if "DL_PUT" in c:
            print(
                "pack_s4",
                "_pack_multi" in c,
                "教案.docx" in c and 'else "教案.docx"' in c,
                "文档.docx" in c,
            )
    for n in g2["nodes"]:
        d = n.get("data") or {}
        if d.get("type") == "llm":
            ...
            if pt and isinstance(pt, list):
                t = (pt[0] or {}).get("text") or ""
            print(
                "sys_has_multi",
                "一次多产物" in t or "多产物" in t,
                "拒假" in t or "工单" in t,
                "len",
                len(t),
            )
